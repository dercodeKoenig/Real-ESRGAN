...
class DiTBlock(nn.Module):
    def __init__(self, dim, heads=12, dim_head=64, t_hidden_dim=1024):
        super().__init__()
        # Normalization
    ...
    def forward(self, x, t_emb):
        """
        x: (B, C, H, W)
        t_emb: (B, t_hidden_dim) - already processed via a TimeStep embedder
        """
...
class RRDB_UNet_v9_t(nn.Module):

    def __init__(self, num_in_ch, num_out_ch, highway_channels_base=32, processing_channels_base=16, num_grow_ch_base=8,
                 encoder_blocks=4, decoder_blocks=6, ae_channel_multipliers = [1,2,4,8], body_rrdb_blocks=12, body_dit_blocks=4, dit_heads=8, dit_head_dim=128, t_hidden_dim = 1024, memory_efficient_inference_device = None, inference = False, checkpoint = False):

        super(RRDB_UNet_v9_t, self).__init__()
        ...
        print("dit_heads", dit_heads)
        print("dit_head_dim", dit_head_dim)
