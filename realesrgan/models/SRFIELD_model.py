...
class SRFIELD(SRModel):

    def __init__(self, opt):
        super(SRFIELD, self).__init__(opt)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(SRFIELD, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast('cuda', dtype=torch.bfloat16):
            # The model predicts the difference to get gt
            pred = self.net_g(model_input)
            
            # Standard Flow Matching loss is MSE
            l_total = self.cri_pix(pred, target)
